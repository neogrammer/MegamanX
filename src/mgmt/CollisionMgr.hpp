...
struct CollisionMgr
{
	...
	static bool PointVsRect(const vf2d& p, rect& r);
	...
	static bool RayVsRect(const vf2d& ray_origin, const vf2d& ray_dir, rect& target, vf2d& contact_point, vf2d& contact_normal, float& t_hit_near);
	...
	// in the algorithm
	static bool DynamicRectVsRect(rect& in, rect& target, vf2d& contact_point, vf2d& contact_normal, float& contact_time, const sf::Time& fElapsedTime);
	// 
	...
	// 
	// 
	//   if (DynamicRectVsRect(vRect[0], vRect[i], cp, cn, ct, l_dt))
	//   {
	//		z.push_back({ i, ct });
	//	 }
	// 
